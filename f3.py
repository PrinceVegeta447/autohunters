...
async def command_handler(event):
    ...
    if command == ".stop":
        ...
        if not bot_running:
            bot_running = True
            await event.reply("Bot has been started. Resuming operations.")
        else:
        ...
        await event.reply(f"The bot is currently {status}. Cooldown: {cooldown} seconds.")

# Example usage of bot_running to control behavior
@client.on(events.NewMessage(from_users=572621020))
async def hunt_or_pass(event):
    global bot_running
