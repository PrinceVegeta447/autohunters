...
async def retry_hunt():
    ...
    for attempt in range(3):  # Retry up to 3 times
        await client.send_message(572621020, '/hunt')
        print(f"Attempt {attempt + 1}: Sent /hunt")
        last_hunt_time = datetime.now()
        await asyncio.sleep(2)  # Wait 2 seconds for a response

        # Check if no bot message was received within the 2-second window
        if datetime.now() - last_bot_message_time > timedelta(seconds=2):
            print("No response detected. Retrying...")
        else:
            print("Response detected. Exiting retry loop.")
            break
