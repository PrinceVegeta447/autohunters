...
# Replace with your Telegram user ID(s) for security
...
async def command_handler(event):
    ...
    # Handle commands
    if command == "/stop":
        await event.reply("Stopping the bot...")
        await client.disconnect()
    elif command.startswith("/set_cooldown"):
        try:
            global cooldown
            cooldown = int(command.split(" ")[1])
            await event.reply(f"Cooldown updated to {cooldown} seconds.")
        except (IndexError, ValueError):
            await event.reply("Invalid usage. Example: /set_cooldown 2")
    elif command == "/status":
        await event.reply(f"The bot is running. Current cooldown: {cooldown} seconds.")
    else:
        await event.reply("Unknown command. Available commands:\n"
                          "/stop - Stop the bot\n"
                          "/set_cooldown <seconds> - Set cooldown time\n"
