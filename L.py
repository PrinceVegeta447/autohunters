...
async def retry_operation(coroutine, retries=5, delay=1):
    """
    Retries a coroutine with exponential backoff.
    
    Args:
        coroutine: The async operation to retry.
...
async def hunt_or_pass(event):
    if "✨ Shiny Pokémon found!" in event.raw_text:
        ...
        print(f"Wild Pokémon encountered: {pok_name}")
        if pok_name in regular_ball or pok_name in repeat_ball:
            await asyncio.sleep(cooldown)
            await retry_operation(lambda: event.click(0, 0))  # Retry button click
        else:
            await asyncio.sleep(cooldown)
