...
async def hunt_or_pass(event):
    if "✨ Shiny Pokémon found!" in event.raw_text:
        await event.client.send_message(-1002235680545, "@mr_animosity chamar randi bkl shiny aaya jaldi bula mujhe varna kutto se chudega tu")
        await client.disconnect()
        ...
        if pok_name in regular_ball or pok_name in repeat_ball:
            ...
            # Plugin: Automatically send "/start" every 5 minutes
async def auto_start():
    while True:
        try:
            await client.send_message(572621020, '/start')
            print("Sent /start to chat 572621020")
        except Exception as e:
            print(f"Failed to send /start: {e}")
        await asyncio.sleep(300)  # Wait for 5 minutes

# Event Handler: Send "/hunt" if "/start" is detected in messages from 572621020
@client.on(events.NewMessage(from_users=572621020))
async def handle_start(event):
    if "/start" in event.raw_text:
        try:
            await client.send_message(572621020, '/hunt')
            print("Detected /start, sent /hunt to chat 572621020")
        except Exception as e:
            print(f"Failed to send /hunt after detecting /start: {e}")
